...
$stmt = $pdo->prepare("INSERT INTO webhook_events (id, event, session, engine, environment_version, environment_engine, environment_tier, environment_browser) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
$stmt = $pdo->prepare("INSERT INTO webhook_users (id, pushName) VALUES (?, ?)");
...
$stmt = $pdo->prepare("INSERT INTO webhook_payloads (id, event_id, timestamp, from_user, to_user, body, hasMedia, ack, ackName) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
$stmt = $pdo->prepare("INSERT INTO webhook_message_data (id, payload_id, viewed, message_body, message_type, timestamp, notifyName, from_user, to_user, ack, invis, isNewMsg, star, kicNotified, recvFresh, isFromTemplate, labels) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
$stmt = $pdo->prepare("INSERT INTO webhook_message_secrets (message_data_id, secret) VALUES (?, ?)");
