...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($count > 0) {
            ...
            $stmt = $pdo->prepare("INSERT INTO users (username, email, password, salt, token, date, time, stat, name, country, phone, full_phone, user_type, user_id) VALUES (:username, :email, :password, :salt, :token, :date, :time, '0', :name, :country, :phone, :full_phone, 'agent', :user_id)");
            $stmt->execute([
                ':username' => $username,
                ':email' => $agent_email,
